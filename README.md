...
命名约定：**`app/`** = 打开 App 后看到的 Compose 界面；**`homewidget/`** = 桌面小组件（RemoteViews + 闹钟 + 前台服务）。二者 UI 实现不同，故不用笼统的 `ui/` 包名。
...
HomeWidgetProvider / AlarmManager
        ↓
HomeWidgetRefreshService（前台）→ HomeWidgetRefreshRunner → AgentRepository.widgetRun
        ↓
...
└── homewidget/                # 桌面小组件（RemoteViews，非 Compose）
    ...
    ├── HomeWidgetAlarmScheduler.kt
    ├── HomeWidgetAlarmReceiver.kt
    ├── HomeWidgetRefreshService.kt
    ├── HomeWidgetRefreshRunner.kt
    ├── HomeWidgetCoordinator.kt
    ├── HomeWidgetLayoutSelector.kt
    ├── HomeWidgetRemoteBinder.kt
    ├── HomeWidgetSystemPermissions.kt
...
```
闹钟 / 手动 ↻ / 开机
  → HomeWidgetRefreshService（前台通知）
  → HomeWidgetRefreshRunner（检查 TTL → widgetRun）
  → WidgetCache.saveSuccess → renderAllWidgets
  → 定时结束后登记下一次闹钟
```
...
| 定时 | 仅 `enabled=true` 的任务登记 [AlarmManager](homewidget/HomeWidgetAlarmScheduler.kt) |
